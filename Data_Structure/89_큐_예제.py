...
def replaceData(numData): # numData	2차원 정수 배열
    ...
    for row in range(ARRAY_LENGTH):
        ...
        for col in range(ARRAY_LENGTH):
            if retData[row][col] < 0:
                retData[row][col] = 0
            if retData[row][col] > 100:
                retData[row][col] = retData[row][col] % 100
    ###########   <-------------- 여기까지 코딩 (1)
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    ###########   여기부터 코딩 (2) ---------------->
    aryData = []
    for row in range(ARRAY_LENGTH-1):
        for col in range(ARRAY_LENGTH-1):
            hap = numData[row][col]+numData[row+1][col]+\
                  numData[row][col+1]+numData[row+1][col+1]
            if hap > maxSum:
                maxSum = hap
