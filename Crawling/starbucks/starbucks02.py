...
if __name__ == '__main__':
    ...
    for SiDo in getSiDo().keys():
        if SiDo == '17':
            total_stores_list = total_stores_list + getStore(sido_code='17')
        else:
            for GuGun in getGuGun(SiDo).keys():
                total_stores_list = total_stores_list + getStore(gugun_code=GuGun)

    total_stores_dict = dict()
    total_stores_dict['list'] = total_stores_list

    total_stores_json = json.dumps(total_stores_dict, ensure_ascii=False)
