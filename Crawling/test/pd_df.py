...
def getCasualties(loc):
    ...
    if loc == csvData1:
        csvHead = ['발생일', '사망자수', '중상자수', '경상자수', '부상신고자수',
                   '발생지_시도', '발생지_시군구', '피해자_당사자종별', '도로형태', '주야', '발생시간']
        fileName = jsonData1
    elif loc == csvData2:
        csvHead = ['발생년월일시', '사망자수', '중상자수', '경상자수', '부상신고자수',
                   '발생지시도', '발생지시군구', '피해자_당사자종별', '도로형태', '주야']
        fileName = jsonData2
    else:
        print('형식에 맞는 csv파일인지 확인하세요')
        return
    df1 = df[csvHead]
    df2 = df1.loc[df1[csvHead[7]] == '보행자']             # 피해자_당사자종별 중 보행자만
    df3 = df2.loc[df2[csvHead[5]] == '서울']               # 발생지_시도 중 서울만
    # df3 = df2_1.loc[df2_1[csvHead[8]].isin(['횡단보도상', '교차로횡단보도내', '횡단보도부근'])]
    ...
    if loc == csvData1:
        for date in dates:        # date가 2015라면 2015-01-01부터 2015-12-31까지의 df를 가지고 옴
            df_1 = df3.set_index([csvHead[0]])
            tdf1 = df_1.loc[f'{date}-01-01':f'{date}-12-31']
            tDict = dict()
            for time in times:
                tdf2 = tdf1.loc[tdf1[csvHead[10]] == time]
                tDict[time] = int(tdf2['사망자수'].sum() + tdf2['중상자수'].sum() +
                                  tdf2['경상자수'].sum() + tdf2['부상신고자수'].sum())

            yearDict[date] = tDict

    timeDict['시간대'] = yearDict



    gangnam_casualties['서울'] = guDict
    total_stores_json = json.dumps(gangnam_casualties, ensure_ascii=False)
    with open(csvLoc+fileName, 'w', encoding='utf_8') as f:
        f.write(total_stores_json)

    time_json = json.dumps(timeDict, ensure_ascii=False)
    with open(csvLoc+'times.json', 'w', encoding='utf_8') as f:
        f.write(time_json)
...
def barGraph():
    ...
    #     plt.text(persent[num], height, '%.1f' % height, ha='center', va='bottom', size=30)
    #     num = num + 1

    # for i, v in enumerate(x):
    #     plt.text(v, accident1[i], persent[i],  # 좌표 (x축 = v, y축 = y[0]..y[1], 표시 = y[0]..y[1])
    #              fontsize=9,
    #              color='blue',
...
def timePlot():
    with open('times.json', 'r', encoding='utf_8') as f:
        casualty = json.load(f)

    years = casualty['시간대']
    yearList = ['2015', '2016', '2017', '2018', '2019']
    colors = ['#d0504b', '#ffd400', '#c1c724', '#56b6c3', '#074862']
    timesList = list(map(str, range(24)))
    ...
    for year in yearList:
        ...
        for time in timesList:
            timesNum.append(years[year][time])
        yearsDict[year] = timesNum

    fig, ax = plt.subplots()
    # 노랑 사상자 빨강 사망자 파랑 사망률
    ax.plot(timesList, yearsDict['2015'], color=colors[0], label='2015')
    ax.plot(timesList, yearsDict['2016'], color=colors[1], label='2016')
    ax.plot(timesList, yearsDict['2017'], color=colors[2], label='2017')
    ax.plot(timesList, yearsDict['2018'], color=colors[3], label='2018')
    ax.plot(timesList, yearsDict['2019'], color=colors[4], label='2019')
    plt.grid(axis='y')  # 격자 y축만 표시되도록
    plt.yticks(list(range(0, 151, 10)))  # y축 범위 0~90까지 10씩 증가하도록
    plt.legend(loc=1)  # 범례위치 upper right
    plt.title('시간대별 사상자')

    df = pd.DataFrame(yearsDict)
    ...
    # plt 파일로 저장
    plt.savefig(f'시간대별.png')
    plt.clf()


if __name__ == '__main__':
    # getCasualties(csvData2)
    # getCasualties(csvData1)
