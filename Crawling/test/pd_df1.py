...
jsonData1 = 'seoul_Casualty1.json'           # csvData1로 만든 json 파일
jsonData2 = 'seoul_dead1.json'               # csvData2로 만든 json 파일
...
def getCasualties(loc):
    ...
    if loc == csvData1:
        csvHead = ['발생일', '사망자수', '중상자수', '경상자수', '부상신고자수',
                   '발생지_시도', '발생지_시군구', '피해자_당사자종별', '도로형태', '주야']
        fileName = jsonData1
    elif loc == csvData2:
    ...
    for gu in guList:
        ...
        for date in dates:        # date가 2015라면 2015-01-01부터 2015-12-31까지의 df를 가지고 옴
            ...
            for time in times:
                numDict = dict()
                df_2 = df_1_1.loc[df_1_1['주야'] == time]
                numDict['사망자수'] = int(df_2['사망자수'].sum())
                numDict['중상자수'] = int(df_2['중상자수'].sum())
                numDict['경상자수'] = int(df_2['경상자수'].sum())
                numDict['부상신고자수'] = int(df_2['부상신고자수'].sum())
                timeDict[time] = numDict
            yearDict[date] = timeDict
...
def barGraph():
    ...
    for time in times:
        ...
        for year in yearList:
            ...
            sum2 = 0
            for gu in guList:
                sum1 = sum1 + gugun2[gu][year][time]['사망자수'] + gugun1[gu][year][time]['중상자수'] + \
                       gugun1[gu][year][time]['경상자수'] + gugun1[gu][year][time]['부상신고자수']
                sum2 = sum2 + gugun2[gu][year][time]['사망자수']
            ...
            if sum1 != 0:
                ...
                a = 0
            persent.append(a)
        timeDict[time] = [accident1, accident2, persent]
    ...
    # 바생성
    ax.bar(x, timeDict['주'][0], color='#ffd400', width=0.3, zorder=3, label='사상자[주]')
    ax.bar(x, timeDict['주'][1], color='#ff7b5a', width=0.3, zorder=3, label='사망자[주]')
    ax.bar(x+0.3, timeDict['야'][0], color='#ccc000', width=0.3, zorder=3, label='사상자[야]')
    ax.bar(x+0.3, timeDict['야'][1], color='#dd2c00', width=0.3, zorder=3, label='사망자[야]')
